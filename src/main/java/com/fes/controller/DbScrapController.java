...
import com.fes.utill.MatcherUtil;
import com.fes.utill.StringUtil;
...
public class DbScrapController {
	...
	public String scrap(DbScrapVO param) {
		...
		if(StringUtil.isNotEmpty(param.getCategory2())) { //아이템 대분류가 있는 경우. 카테고리2에 공백을 잘라냄, 빈 문자열인지 확인.공백이 아니면 true반환
			...
			log.info("♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥category2="+param.getCategory2());
...
//		if(param.getCategory3() != null && !"".equals(param.getCategory2().trim()))
		...
		if(StringUtil.isNotEmpty(param.getPage())){
			log.info("♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥page="+param.getPage());
...
//			e.printStackTrace();
		...
		if(i > 1) {
			...
			try {
				...
				System.out.println("PageURL 형식이 잘못 되었습니다.");
//				e.printStackTrace();
			}
		}
...
//						System.out.println(evo);
					...
					// http://localhost:8080/scrap?category2=3&category3=11&min_gear_lv=81&max_gear_lv=90
					// http://localhost:8080/scrap?category2=3&category3=34&page=1
					}
				}, executorService);
